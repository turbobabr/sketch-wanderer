...
Utils.readJSON = function (filePath) {
  var str = NSString.stringWithContentsOfFile_encoding_error(filePath, NSUTF8StringEncoding, null);
  if (!str) {
    return null;
  }
...
Utils.writeJSON = function (filePath, obj) {
  var prettyPrint = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;

  var str = prettyPrint ? JSON.stringify(obj, null, 4) : JSON.stringify(obj);
...
Utils.currentDocument = function () {
  return MSDocument.currentDocument();
};

Utils.currentCommand = function () {
  return coscript.printController();
};

Utils.currentPluginBundle = function () {
  return Utils.currentCommand().pluginBundle();
};

Utils.manifestFilePath = function () {
  var pluginBundle = Utils.currentPluginBundle();
  return String(Utils.normalize(pluginBundle.url().path())) + '/Contents/Sketch/manifest.json';
};

Utils.runForever = function () {
  coscript.setShouldKeepAround(true);
};

Utils.stopRunningForever = function () {
  coscript.setShouldKeepAround(false);
};

Utils.refreshLayerList = function () {
  Utils.layerListViewController().refresh();
};

Utils.filterArray = function (array, predicateFormat) {
  return array.filteredArrayUsingPredicate(NSPredicate.predicateWithFormat(predicateFormat));
};

Utils.findOne = function (array, predicateFormat) {
  return Utils.filterArray(array, predicateFormat).firstObject();
};

Utils.showMessage = function (msg) {
  var document = Utils.currentDocument();
  document.showMessage('[walker]: ' + String(msg));
};

Utils.openUrlInDefaultBrowser = function (url) {
  NSWorkspace.sharedWorkspace().openURL(NSURL.URLWithString(url));
};

Utils.layerListViewController = function () {
  return Utils.currentDocument().valueForKeyPath('sidebarController.layerListViewController');
};

Utils.layerListView = function () {
...
var GroupExpandedType = exports.GroupExpandedType = {
  Undecided: 0,
  Collapsed: 1,
  Expanded: 2
};

var ShortcutsSchema = exports.ShortcutsSchema = {
  Default: 'default',
  ControlOnly: 'controlOnly'
};

var TargetGroupType = exports.TargetGroupType = {
  Selection: 'selection',
  Artboard: 'artboard',
...
var CanvasAction = exports.CanvasAction = {
  None: 'none',
  Center: 'center',
  ZoomToFit: 'center'
};
...
var keyForProperty = function keyForProperty(prop) {
  return 'com.turbobabr.sketch.wanderer.' + String(prop);
};

var LAST_ACTION_KEY = keyForProperty('last-action');
var BASE_ROW_KEY = keyForProperty('base-row');
var BERSERK_BASE_ROW_KEY = keyForProperty('berserk-base-row');
...
var ActionsManager = function () {
  ...
  _createClass(ActionsManager, [{
    ...
    value: function () {
      function get(prop) {
        return _utils2['default'].normalize(this.ThreadDictionary[prop]);
      }
...
exports['default'] = new ActionsManager();
...
exports['default'] = function (context) {
  var commandIdentifier = _utils2['default'].normalize(context.command.identifier());
...
var _expand2 = __webpack_require__(16);
...
var _selectArtboard = __webpack_require__(19);

var _selectArtboard2 = _interopRequireDefault(_selectArtboard);

var _switchShortcutsSchema = __webpack_require__(20);

var _switchShortcutsSchema2 = _interopRequireDefault(_switchShortcutsSchema);

var _actionsManager = __webpack_require__(6);

var _actionsManager2 = _interopRequireDefault(_actionsManager);
...
var commandHandlers = {
  moveUp: function () {
    function moveUp(context) {
      (0, _move2['default'])(context, _constants.Direction.Up);
      _actionsManager2['default'].setLastAction(_actionsManager.ActionType.MoveUp);
    }
  ...
  moveDown: function () {
    function moveDown(context) {
      (0, _move2['default'])(context, _constants.Direction.Down);
      _actionsManager2['default'].setLastAction(_actionsManager.ActionType.MoveDown);
    }

    return moveDown;
  }(),
  selectUp: function () {
    function selectUp(context) {
      (0, _select2['default'])(context, _constants.Direction.Up);
      _actionsManager2['default'].setLastAction(_actionsManager.ActionType.SelectUp);
    }

    return selectUp;
  }(),
  selectDown: function () {
    function selectDown(context) {
      (0, _select2['default'])(context, _constants.Direction.Down);
      _actionsManager2['default'].setLastAction(_actionsManager.ActionType.SelectDown);
    }

    return selectDown;
  }(),
  selectArtboardAbove: function () {
    function selectArtboardAbove(context) {
      (0, _selectArtboard2['default'])(context, _constants.Direction.Up);
      _actionsManager2['default'].setLastAction(_actionsManager.ActionType.SelectArtboardAbove);
    }

    return selectArtboardAbove;
  }(),
  selectArtboardBelow: function () {
    function selectArtboardBelow(context) {
      (0, _selectArtboard2['default'])(context, _constants.Direction.Down);
      _actionsManager2['default'].setLastAction(_actionsManager.ActionType.SelectArtboardBelow);
    }

    return selectArtboardBelow;
  }(),
  expand: function () {
    function expand(context) {
      (0, _expand3['default'])(context, _constants.TargetGroupType.Selection);
      _actionsManager2['default'].setLastAction(_actionsManager.ActionType.Expand);
    }

    return expand;
  }(),
  expandArtboard: function () {
    function expandArtboard(context) {
      (0, _expand3['default'])(context, _constants.TargetGroupType.ArtboardGraph);
      _actionsManager2['default'].setLastAction(_actionsManager.ActionType.Expand);
    }

    return expandArtboard;
  }(),
  collapse: function () {
    function collapse(context) {
      (0, _collapse3['default'])(context, _constants.TargetGroupType.Selection);
      _actionsManager2['default'].setLastAction(_actionsManager.ActionType.Collapse);
    }

    return collapse;
  }(),
  collapseArtboard: function () {
    function collapseArtboard(context) {
      (0, _collapse3['default'])(context, _constants.TargetGroupType.ArtboardGraph);
      _actionsManager2['default'].setLastAction(_actionsManager.ActionType.Collapse);
    }

    return collapseArtboard;
  }(),
  controlOptionSchema: function () {
    function controlOptionSchema(context) {
      (0, _switchShortcutsSchema2['default'])(_constants.ShortcutsSchema.Default);
    }

    return controlOptionSchema;
  }(),
  controlSchema: function () {
    function controlSchema(context) {
      (0, _switchShortcutsSchema2['default'])(_constants.ShortcutsSchema.ControlOnly);
    }
  ...
  help: function () {
    function help(context) {
      _utils2['default'].openUrlInDefaultBrowser('https://github.com/turbobabr/sketch-wanderer');
    }
...
var runForeverBlackList = ['controlOptionSchema', 'controlSchema', 'help'];
var runCommand = function runCommand(id, contex) {
  if (!commandHandlers[id]) {
    console.warn('[sketch-wanderer]: Unknown command - \'' + String(id) + '\'');
    return;
  }

  if (!(0, _lodash.includes)(runForeverBlackList, id)) {
    _utils2['default'].runForever();
  }
...
var move = function move(context, direction) {
  var view = _utils2['default'].layerListView();
  var index = view.selectedRow();
  var currentLayer = view.itemAtRow(index);

  index = index != -1 ? index : 0;

  switch (direction) {
    case _constants.Direction.Up:
      index--;
      break;

    case _constants.Direction.Down:
      index++;
      break;
  }

  var numberOfRows = view.numberOfRows();
  if (index < 0) {
  ...
  if (currentLayer && targetLayer && direction == _constants.Direction.Up && currentLayer.valueForKeyPath('parentGroup.objectID') == targetLayer.objectID() && targetLayer.layerListExpandedType() == _constants.GroupExpandedType.Undecided) {
    console.log("Have to expand item directly!");
    targetLayer.layerListExpandedType = _constants.GroupExpandedType.Expanded;
  }
...
var expand = function expand(context, target) {
  var selection = context.selection;


  if (target == _constants.TargetGroupType.ArtboardGraph) {
    ...
    _lodash2['default'].each(_utils2['default'].normalize(artboards), function (artboard) {
      ...
      _lodash2['default'].each(_utils2['default'].normalize(targetGroups), function (layer) {
        layer.layerListExpandedType = _constants.GroupExpandedType.Expanded;
      });
    });

    _utils2['default'].refreshLayerList(context);
  } else if (target == _constants.TargetGroupType.Selection) {
    ...
    if (layer && layer.isKindOfClass(MSLayerGroup) && layer.expandableInLayerList() && !layer.isExpanded()) {
      ...
      var predicate = NSPredicate.predicateWithFormat('(SELF isKindOfClass:%@) AND (expandableInLayerList == TRUE) AND (isExpanded == FALSE)', MSLayerGroup['class']());
      _lodash2['default'].each(_utils2['default'].normalize(parent.layers().filteredArrayUsingPredicate(predicate)), function (layer) {
        layer.layerListExpandedType = _constants.GroupExpandedType.Expanded;
      });
...
var collapse = function collapse(context, target) {
  var selection = context.selection,
  ...
  if (target == _constants.TargetGroupType.ArtboardGraph) {
    var targetArtboards = _utils2['default'].normalize(selection.valueForKeyPath("@distinctUnionOfObjects.parentArtboard"));
    _.each(targetArtboards, function (artboard) {
      var predicate = NSPredicate.predicateWithFormat('(SELF isKindOfClass:%@) AND (expandableInLayerList == TRUE) AND (isExpanded == TRUE)', MSLayerGroup['class']());
      var targetGroups = artboard.children().filteredArrayUsingPredicate(predicate);
      _.each(_utils2['default'].normalize(targetGroups), function (layer) {
        return layer.layerListExpandedType = _constants.GroupExpandedType.Collapsed;
      });
    });

    var resultingResponder = currentArtboard || _.first(targetArtboards);
    ...
    var layer = selection.firstObject();
    if (layer && layer.isKindOfClass(MSLayerGroup) && layer.expandableInLayerList() && layer.isExpanded()) {
      layer.layerListExpandedType = _constants.GroupExpandedType.Collapsed;
      _utils2['default'].refreshLayerList(context);
    } else if (layer) {
      var parent = layer.parentGroup();
      ...
      parent.layerListExpandedType = _constants.GroupExpandedType.Collapsed;
      parent.select_byExpandingSelection(true, false);
      _utils2['default'].refreshLayerList(context);
    }
  } else if (target == _constants.TargetGroupType.PageGraph) {
    // TODO: TO IMPLEMENT
  }
};
...
var select = function select(context, direction) {
  var view = _utils2['default'].layerListView();
  if (!_lodash2['default'].includes([_actionsManager.ActionType.SelectDown, _actionsManager.ActionType.SelectUp], _actionsManager2['default'].lastAction)) {
    _actionsManager2['default'].baseRowIndex = view.selectedRow();
  }

  _actionsManager2['default'].berserkBaseRowIndex = view.selectedRow();
  var baseRowIndex = _actionsManager2['default'].baseRowIndex;

  var selectedIndexes = view.selectedRowIndexes();
  var firstSelectedIndex = selectedIndexes.firstIndex();
  var lastSelectedIndex = selectedIndexes.lastIndex();
  ...
  var deselectIndex = function deselectIndex(index) {
    var item = view.itemAtRow(index);
    if (item) {
      item.select_byExpandingSelection(false, true);
    }
  };
  ...
  } else if (direction == _constants.Direction.Down) {
...
var performCanvasAction = function performCanvasAction(targetArtboard, action) {
  if (action == _constants.CanvasAction.None) {
    return;
  }

  switch (action) {
    case _constants.CanvasAction.Center:
      // TODO: To implement
      break;

    case _constants.CanvasAction.ZoomToFit:
      // TODO: To implement
      break;
  }
};

var selectArtboard = function selectArtboard(context, direction) {
  var action = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : _constants.CanvasAction.None;
  ...
  var isPartOfSelection = _utils2['default'].findOne(selection, 'objectID == \'' + String(currentArtboard.objectID()) + '\'') != null;
  if (!isPartOfSelection && direction == _constants.Direction.Up) {
    currentArtboard.select_byExpandingSelection(true, false);
    return;
  }

  var artboards = _lodash2['default'].reverse(_utils2['default'].normalize(page.artboards()));
  var index = _lodash2['default'].findIndex(artboards, function (ab) {
    return ab.objectID() == currentArtboard.objectID();
  });
  if (index == -1) {
    // TODO: Add warning message.
    return;
...
var switchShortcutsSchema = function switchShortcutsSchema(schema) {
  var symbolsMap = {
    moveUp: KeyMap.Up,
  ...
  var modifiers = schema == _constants.ShortcutsSchema.Default ? [KeyMap.Control, KeyMap.Options] : [KeyMap.Control];
  _.each(symbolsMap, function (value, key) {
    var shortcut = modifiers.concat([value]).join(' ');
    commandsPatch[key] = { shortcut: shortcut };
  });

  commandsPatch['controlOptionSchema'] = { name: schema === _constants.ShortcutsSchema.Default ? "✓  control-option+arrows" : "     control-option+arrows" };
  commandsPatch['controlSchema'] = { name: schema !== _constants.ShortcutsSchema.Default ? "✓  control+arrows" : "     control+arrows" };

  var filePath = _utils2['default'].manifestFilePath();
  var manifest = _utils2['default'].readJSON(filePath);

  manifest = _.assign({}, manifest, {
    commands: _.map(manifest.commands, function (command) {
  ...
  _utils2['default'].writeJSON(filePath, manifest);
};

exports['default'] = switchShortcutsSchema;
